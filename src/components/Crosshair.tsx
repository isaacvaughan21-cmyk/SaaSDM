      ...
      <svg width={300} height={300} viewBox={`0 0 ${VB} ${VB}`} className="overflow-visible">
        <defs>
          ...
          <radialGradient id="dotFill" cx="35%" cy="35%" r="70%">
            <stop offset="0%" stopColor="#46423B" />
            <stop offset="100%" stopColor="#1B1A17" />
          </radialGradient>
        </defs>

        {/* quadrant tints: bottom-left = weak, top-right = strong */}
        <rect x={x0} y={y0} width={mid} height={mid} fill="#F4F2EC" />
        <rect x={x0 + mid} y={y0} width={mid} height={mid} fill="#dcefe8" />
        <rect x={x0} y={y0 + mid} width={mid} height={mid} fill="#f7e3d6" />
        <rect x={x0 + mid} y={y0 + mid} width={mid} height={mid} fill="#F4F2EC" />

        {/* diagonal guide (scores plot along this) */}
        <line x1={x0} y1={y1} x2={x1} y2={y0} stroke="#D8D2C6" strokeWidth={1} strokeDasharray="4 4" />

        {/* frame + axes */}
        <rect x={x0} y={y0} width={inner} height={inner} fill="none" stroke="#D8D2C6" />
        ...
        {/* corner labels */}
        <text x={x0 + mid * 0.5} y={y0 + 16} textAnchor="middle" fontSize="9" className="fill-muted" fontWeight="600">{quadrants[0]}</text>
        ...
        <text x={x0 + mid * 1.5} y={y1 - 8} textAnchor="middle" fontSize="9" className="fill-muted" fontWeight="600">{quadrants[3]}</text>
        ...
        {/* axis captions */}
        <text x={x0 + mid} y={VB - 6} textAnchor="middle" fontSize="10" className="fill-muted">{xLabel}</text>
        <text x={-(y0 + mid)} y={16} textAnchor="middle" fontSize="10" className="fill-muted" transform="rotate(-90)">{yLabel}</text>
      </svg>
    </div>
  );
}
